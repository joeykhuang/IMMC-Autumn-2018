...
for a in range(37):
	...
	if popEachYear != []:
		if a > 21:
			total = factor * cityStartPop + popEachYear[-1] * 0.011 * a
		elif 5 <= a <= 21:
			total = factor * cityStartPop + popEachYear[-1] * 0.009 * a
		else:
			total = factor * cityStartPop + popEachYear[-1] * 0.0015 * a
	else:
		total = cityStartPop
...
np.savetxt('1979-2015_PopulationModelBeijingOnly.csv', (years, popEachYear[:37]), fmt='%10.5f', delimiter=',')
print cityCarryCap
print popEachYear[37]
# Using matplotlib to graph the population
plt.plot(range(startingCalcYear + yearsAfter79, 2015+1), popEachYear[yearsAfter79:37])
